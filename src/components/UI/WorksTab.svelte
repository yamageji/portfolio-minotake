...
<div class="w-full">
  <ul
    ...
    class="flex gap-4 border-b-2 border-slate-200 font-barlow-semi text-2xl font-semibold text-slate-600"
  >
    {#each data as { id, title }, index}
      ...
      <li role="presentation" class="-mb-0.5">
        <button
          ...
          on:click={onClick}
          class="relative block border-b-2 border-slate-200 px-2 py-2 tracking-wider duration-150 hover:text-slate-900 focus:text-slate-900 aria-selected:border-teal-500 aria-selected:text-slate-900"
        >
          {title}
        </button>
      </li>
